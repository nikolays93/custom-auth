...
function retrieve_password() {
	...
	$message .= '<' . network_site_url( LOGINPAGE . "?action=resetpass&key=$key&login=" . rawurlencode( $user_login ), 'login' ) . ">\r\n";
...
case 'logout' :
	...
	if ( ! empty( $_REQUEST['redirect_to'] ) ) {
		...
		$redirect_to = LOGINPAGE . '?loggedout=true';
...
case 'retrievepassword' :
	...
	if ( $http_post ) {
		...
		if ( !is_wp_error($errors) ) {
			$redirect_to = !empty( $_REQUEST['redirect_to'] ) ? $_REQUEST['redirect_to'] : LOGINPAGE . '?checkemail=confirm';
			wp_safe_redirect( $redirect_to );
			exit();
		}
	}
	...
	login_header(__('Lost Password'), '<p class="message">' .
		apply_filters('lost_password_login_header_text', __('Please enter your username or email address. You will receive a link to create a new password via email.')) . '</p>', $errors);
...
case 'rp' :
	...
	if ( ! $user || is_wp_error( $user ) ) {
		...
		if ( $user && $user->get_error_code() === 'expired_key' )
			wp_redirect( site_url( LOSTPWDPAGE . '?error=expiredkey' ) );
		else
			wp_redirect( site_url( LOSTPWDPAGE . '?error=invalidkey' ) );
		exit;
	}
...
case 'register' :
	...
	if ( !get_option('users_can_register') ) {
		wp_redirect( site_url(LOGINPAGE . '?registration=disabled') );
		exit();
	}
	...
	if ( $http_post ) {
		...
		if ( !is_wp_error($errors) ) {
			$redirect_to = !empty( $_POST['redirect_to'] ) ? $_POST['redirect_to'] : LOGINPAGE . '?checkemail=registered';
			wp_safe_redirect( $redirect_to );
			exit();
		}
	}
